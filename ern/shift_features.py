...
class ShiftFeatures(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, sliding_window_features, eval_position='past', step_sizes=1, drop_na=True):

        self.sliding_window_features = sliding_window_features
        if eval_position == 'both': eval_position = ['future', 'past']
        if not isinstance(eval_position, list): eval_position = [eval_position]
            
        self.eval_position = eval_position
        if not isinstance(step_sizes, list): step_sizes = [step_sizes]
            ...
            for step_size in self.step_sizes:
                for shift_direction in self.eval_position:
                    ...
                    if shift_direction == 'past':
                        tmp[step_size:] = df_transformed.loc[:,[feature]][:-step_size].values.T
                        df_transformed[feature + '_t_minus_' + str(step_size)] = tmp
                        
                    if shift_direction == 'future':
                        tmp[0:-step_size] = df_transformed.loc[:,[feature]][step_size:].values.T
    ...
    def get_sliding_window_df(self, eval_pos, df_transformed, drop_na):
        sw = SlidingWindow(window_size=self.window_size, step_size=self.step_size,
             eval_position=eval_pos, timeseries_key_name='TimeUTC', dropna=drop_na)
