...
def get_df(file):
    ...
    cols_to_drop = [
        c
        for c in df.columns
        if "sin" in c.lower() or "cos" in c.lower() or "plus" in c.lower()
    ] + ["DayInMonth", "WeekOfYear", "MaxPowerGeneration"]  
...
def main(data_folder, data_type):
    ...
    output_folder = f"data/{data_type}_cosmo_old/"
    ...
    if "pv" in data_type:
        min_samples = 12 * 30 * 24 

    dfs = {}
    for f in tqdm.tqdm(files):
        df = get_df(f)
        sn = to_short_name(f)
        if "48a57894" in f:  # sn in bl:
            print("skipped")
            continue
